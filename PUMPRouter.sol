...
contract PancakeRouter is IPancakeRouter02 {
    ...
    uint256 private constant FEE_DENOMINATOR = 10**5;
        ...
        returns (uint[] memory amounts)
    {
        ...
        amounts = PancakeLibrary.getAmountsOut(factory, sendAmt, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'PancakeRouter: INSUFFICIENT_OUTPUT_AMOUNT');
        ...
        ensure(deadline)
    {
        ...
        uint sendAmt = msg.value.sub(fee_);
        IWETH(WETH).deposit{value: sendAmt}();
        assert(IWETH(WETH).transfer(PancakeLibrary.pairFor(factory, path[0], path[1]), sendAmt));
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
